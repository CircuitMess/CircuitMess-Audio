...
PlaybackSystem::PlaybackSystem(const i2s_pin_config_t& i2s_pin_config, const fs::File& f) : PlaybackSystem(i2s_pin_config){
	open(f);
}

PlaybackSystem::PlaybackSystem(const i2s_pin_config_t& i2s_pin_config) : audioTask("MixAudio", audioThread, 4 * 1024, this), queue(4, sizeof(PlaybackRequest*)){
	out = new OutputI2S({
