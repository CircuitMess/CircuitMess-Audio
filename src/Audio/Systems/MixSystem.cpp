...
MixSystem::MixSystem(const i2s_pin_config_t& i2s_pin_config, const fs::File& f1, const fs::File& f2) : MixSystem(i2s_pin_config){
	open(0, f1);
	open(1, f2);
}

MixSystem::MixSystem(const i2s_pin_config_t& i2s_pin_config) : audioTask("MixAudio", audioThread, 16 * 1024, this), queue(6, sizeof(MixRequest*)){
	mixer = new Mixer();
